...
function EditableAmount({ value, curr, onSave }) {
  ...
  if (editing) {
    return (
      <input
        ...
        onBlur={() => { setEditing(false); const p = parseFloat(val); if (!isNaN(p) && p >= 0) onSave(p); }}
  ...
  return (
    <span className="font-semibold text-orange-600 cursor-pointer hover:underline" onClick={() => { setVal(String(value ?? "")); setEditing(true); }} title="Click to edit">
...
export default function PayrollAP({ users, households, selectedSeason = "" }) {
  ...
  const exportCSV = () => {
    ...
    let csv = "Line #,Employee,Household,Description,Amount,Date,Paid By,Reimbursable,Receipt,Approved\n";
    rows.forEach(r => {
      csv += `${r.running_id},${esc(r.employee)},${esc(r.household)},${esc(r.description)},${r.amount},${esc(r.date)},${esc(r._paid_by || r.paid_by)},${esc(r.reimbursable ? "Yes" : "No")},${esc(r._receipt_url)},${esc(r.approved)}\n`;
    });
