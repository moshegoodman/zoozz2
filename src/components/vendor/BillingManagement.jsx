...
import StatusEditModal from '../admin/StatusEditModal';
import TotalsDialogs from './billing/TotalsDialogs';
...
export default function BillingManagement({ vendor, vendorId, userType, onRefresh }) {
  ...
  // State for order status change modal
  ...
  const processedOrders = useMemo(() => {
    ...
    let finalFiltered = expandedOrders.filter(order => {
        ...
        if (filters.household && order.household_id !== filters.household) return false;
  ...
  const handleOpenPriceEditor = (order) => setEditingPricesOrder(order);
  ...
  const handleSaveOrderStatusChange = async () => {
    if (!statusEditOrder || !newOrderStatus) return;
    try {
      await Order.update(statusEditOrder.id, { status: newOrderStatus });
  ...
  return (
    <div className="space-y-6">
                ...
                <table className="w-full">
                  <thead className="bg-gray-50">
                    ...
                    <tr className="bg-gray-100">
                        ...
                        <td className={`p-2 border-gray-200 ${isRTL ? 'border-l' : 'border-r'}`}><Select value={filters.household || 'all'} onValueChange={v => handleFilterChange('household', v === 'all' ? '' : v)}><SelectTrigger className="h-8"><SelectValue placeholder={t('common.all')} /></SelectTrigger><SelectContent><SelectItem value="all">{t('common.all')}</SelectItem>{Array.from(new Map((orders||[]).filter(o=>o.household_id).map(o=>{const h=households.find(x=>x.id===o.household_id);return [o.household_id,(language==='Hebrew'&&(h?.name_hebrew||o.household_name_hebrew))||h?.name||o.household_name||o.household_id];})).entries()).sort((a,b)=>String(a[1]).localeCompare(String(b[1]))).map(([id,label])=><SelectItem key={id} value={id}>{label}</SelectItem>)}</SelectContent></Select></td>
                                  ...
                                  {(userType === 'admin' || userType === 'chief of staff') && (
                                    <>
                                      ...
                                      <Button
                                        ...
                                        title={t('admin.orderManagement.changeStatus', 'Change Order Status')}
                                      >
                                        <RefreshCw className="w-4 h-4 ltr:mr-1 rtl:ml-1" />
                                        {t('admin.orderManagement.changeStatus', 'Change Status')}
                                      </Button>
                                      {/*
                                      <Button
                                        variant="outline"
      ...
      {/* Price Editor Modal (extracted into its own component) */}
      <PriceEditorModal
        order={editingPricesOrder}
        language={language}
        t={t}
        formatDeliveryTime={formatDeliveryTime}
        onClose={() => setEditingPricesOrder(null)}
        onSaved={onRefresh}
      />
      <ReturnItemsModal
        isOpen={!!returnOrder}
      ...
      />
      <TotalsDialogs
        showTotalsDialog={showTotalsDialog}
        setShowTotalsDialog={setShowTotalsDialog}
        calculatedTotals={calculatedTotals}
        showShoppedTotalsDialog={showShoppedTotalsDialog}
        setShowShoppedTotalsDialog={setShowShoppedTotalsDialog}
        calculatedShoppedTotals={calculatedShoppedTotals}
        ILS_TO_USD_RATE={ILS_TO_USD_RATE}
      />

      <StatusEditModal
        isOpen={!!statusEditOrder}
        onOpenChange={(open) => { if (!open) handleCancelStatusEdit(); }}
        newStatus={newOrderStatus}
        setNewStatus={setNewOrderStatus}
        onCancel={handleCancelStatusEdit}
        onSave={handleSaveOrderStatusChange}
      />
    </div>
  );
}
