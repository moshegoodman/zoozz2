...
import {
  Plus, Trash2, Languages, Loader2, Image as ImageIcon, Search, X, StickyNote, ArrowUp, ArrowDown } from
'lucide-react';
...
export default function MenuEditor({ menu, allergyText, onSaved, canEdit, isManager }) {
  ...
  const translateChefDish = async (courseIdx, dishIdx) => {
    ...
    {setTranslating((p) => ({ ...p, [dishKey]: false }));}
  };
                  ...
                  <div className="grid grid-cols-[1fr_1fr_1fr] items-start">
                    ...
                    <div className="px-3 py-2.5 bg-gray-50 border-r">
                      <div className="flex items-center justify-between gap-2">
                        <div className="text-sm font-medium text-gray-700">{dish.english || <span className="text-gray-300 italic text-xs">—</span>}</div>
                        {dish.hebrew &&
                     <div className="text-xs text-gray-400 text-right" dir="rtl">{dish.hebrew}</div>
                     }
                      </div>
                      {dish.service_style &&
