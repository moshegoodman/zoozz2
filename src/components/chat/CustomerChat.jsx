            ...
            <motion.div
              ...
              transition={{ type: "tween", duration: 0.28, ease: [0.25, 0.46, 0.45, 0.94] }}>
                ...
                <div key={index} className={`flex flex-col gap-1 ${msg.sender_email === user.email ? 'items-end' : 'items-start'}`}>
                      <div className={`max-w-[80%] p-3 opacity-85 rounded-[20px] ${msg.sender_email === user.email ? 'bg-green-600 text-white' : 'bg-gray-200 text-gray-900'}`}>
    ...
    <>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 h-[calc(100vh-64px)] relative">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 h-full">
          <Card className="flex flex-col max-h-[80vh] px-2">
            <CardHeader>
            ...
            <CardContent className="p-0 flex-grow overflow-hidden flex flex-col">
              <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full flex-grow flex flex-col overflow-hidden">
                <TabsList className="grid w-full grid-cols-2 h-13 rounded-[10px] px-1 py-1">
                  <TabsTrigger value="open" className="py-3">{t('vendor.chat.openChats', 'Open')} ({openChats.length})</TabsTrigger>
